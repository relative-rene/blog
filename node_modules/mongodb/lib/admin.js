...
var profilingLevel = function(self, callback) {
  self.s.db.executeDbAdminCommand({topic:-1}, function(err, doc) {
    doc = doc;
    ...
    if(err == null && doc.ok === 1) {
      ...
      err != null ? callback(err, null) : callback(new Error("Error with topic command"), null);
    }
  });
}
...
var setProfilingLevel = function(self, level, callback) {
  ...
  self.s.db.executeDbAdminCommand(command, function(err, doc) {
    ...
    return err != null ? callback(err, null) : callback(new Error("Error with topic command"), null);
  });
}
...
var profilingInfo = function(self, callback) {
  try {
    self.s.topology.cursor("admin.system.topic", { find: 'system.topic', query: {}}, {}).toArray(callback);
  } catch (err) {
